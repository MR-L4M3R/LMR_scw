...
class main:
 def main():
  ...
  if (hyu == '1') or (hyu == '01'):
   ...
   print (f"{dark}[{dark2}•{dark}] {dark}PILIHAN APA TUH!!")
   main.main()
 def start():
  try:
      nom=str(input(f"{dark}[{dark2}•{dark}] {hm}Nomor Yang Akan Di Siksa (ex:88xx) ~> "))
      if len(nom) <= 5: print (f"{dark}[{dark2}•{dark}] {dark}Ga Ada Nomor Kek Gitu Cok!");main.start()
      print (f"\r{dark}[{dark2}•{dark}] {dark2}Ok, mulai menyiksa....")
      print (f"{dark}[{dark2}•{dark}] {hm}Target > {MR.L4M3R}{nom}")
      ...
      spam.theharvest(nom)
      loding()
      print (f"{dark0}[{dark}•{dark0}] {dark2}Proses selesai")
      sys.exit()
  except ru.ConnectionError:print (f"{dark0}[{dark}•{dark0}]{dark2} Koneksi Error")
  except KeyboardInterrupt:print (f"{dark0}[{dark}•{dark0}]{dark} Script dihentikan")
import itertools
kuni  = '\x1b[33m'
def animate():
    for c in itertools.cycle(['|', '/', '-', '\\']):
        if done:
            break
        sys.stdout.write(f'\r{dark}[{dark2}•{dark}] {dark}Loading {dark}' + c+"\n")
        sys.stdout.flush()
        time.sleep(0.1)
class spam:
 def piza(nom):
 ...
 def mypoin(nom):
     ...
     if e == '{"status": "ok"}':
      print (f"{dark0}[{dark}•{dark0}] {dark0}{kan}({hm}{dark}BOOM SMS{dark2}) {dark0}MyPoin{kan} Berhasil Meledak...")
     else:
      print (f"{dark0}[{dark}•{dark0}] {dark0}{dark2}({hm}{dark}BOOM SMS{dark2}) {dark0}MyPoin{kun} Meledak Di Tempat..")
 def map(nom):
    hd = {
    ...
    if 'error' in pil:print (f"{dark0}[{dark}•{dark0}] {dark0}{dark2}({hm}{dark}BOOM SMS{dark2}) {dark0}Mapclub{kun} Meledak Di Tempat..")
    else:print (f"{dark0}[{dark}•{dark0}] {dark0}{kan}({hm}{dark}BOOM SMS{dark2}) {dark0}Mapclub{kan} Berhasil Meledak...")
 def icq(nom):
...
try:
 os.system("clear")
 time.sleep(2)
 print (f'{dark0}[{dark}•{dark0}] {mr_dark}Subs MR.L4M3RXploit!')
 time.sleep(2)
 os.system('xdg-open https://youtube.com/channel/UCmuNYQZM9crdJ59kUP9khIg')
 time.sleep(2)
 os.system('clear')
 bnr()
 menu=["Mulai","Subscribe MR.L4M3RXploit","keluar"]
 print (f"     By: {tot}MR.L4M3R{sop}        ")
 for i in range(len(menu)):
   print (f"{ken} [0"+str(i+1).ljust(1)+"] \033[31m"+menu[i])
 main.main()
except ru.ConnectionError:print (f"{ken}[{kun}•{ken}] {kun}Koneksi Error")
except KeyboardInterrupt:print (f"{ken}[{kun}•{ken}] {kan} ctrl + c terdeteksi")
